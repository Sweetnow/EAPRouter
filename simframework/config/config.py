"""LLM configuration from environment variables for simframework"""
...
from simframework.logger import get_logger, setup_litellm_logging
...
class Config:
    """
    Configuration from environment variables for simframework.

    This class manages all configuration settings for the simframework framework,
    ...
    Environment Variables:
        SimFramework_HOME_DIR: Base directory for storing agent data and memories
        SimFramework_LLM_API_KEY: API key for the default LLM service
        SimFramework_LLM_API_BASE: Base URL for the default LLM API endpoint
        SimFramework_LLM_MODEL: Model name for the default LLM
        SimFramework_CODER_LLM_API_KEY: API key for code generation LLM (falls back to LLM_API_KEY)
        SimFramework_CODER_LLM_API_BASE: Base URL for code generation LLM API
        SimFramework_CODER_LLM_MODEL: Model name for code generation tasks
        SimFramework_NANO_LLM_API_KEY: API key for high-frequency operations LLM (falls back to LLM_API_KEY)
        SimFramework_NANO_LLM_API_BASE: Base URL for nano LLM API
        SimFramework_NANO_LLM_MODEL: Model name for high-frequency/low-latency operations
        SimFramework_EMBEDDING_API_KEY: API key for embedding model (falls back to LLM_API_KEY)
        SimFramework_EMBEDDING_API_BASE: Base URL for embedding API
        SimFramework_EMBEDDING_MODEL: Model name for text embeddings
        SimFramework_EMBEDDING_DIMS: Dimensionality of embedding vectors
    """

    HOME_DIR: str = os.getenv("SimFramework_HOME_DIR", "./SimFramework_data")
    """
    Base directory path for storing agent data, memories, and persistent files.

    Environment variable: SimFramework_HOME_DIR
    Default: "./SimFramework_data"
    ...
    LLM_API_KEY: Optional[str] = os.getenv("SimFramework_LLM_API_KEY")
    """
    API key for authenticating with the default LLM service.

    Environment variable: SimFramework_LLM_API_KEY
    ...
    LLM_API_BASE: str = os.getenv(
        "SimFramework_LLM_API_BASE", "https://cloud.infini-ai.com/maas/v1"
    )
    """
    Base URL endpoint for the default LLM API service.

    Environment variable: SimFramework_LLM_API_BASE
    ...
    LLM_MODEL: str = os.getenv("SimFramework_LLM_MODEL", "qwen3-next-80b-a3b-instruct")
    """
    Model identifier for the default LLM used in general operations.

    Environment variable: SimFramework_LLM_MODEL
    ...
    CODER_LLM_API_KEY: Optional[str] = (
        os.getenv("SimFramework_CODER_LLM_API_KEY") or LLM_API_KEY
    )
    """
    API key for the code generation LLM service.

    Environment variable: SimFramework_CODER_LLM_API_KEY
    ...
    CODER_LLM_API_BASE: str = os.getenv("SimFramework_CODER_LLM_API_BASE", LLM_API_BASE)
    """
    Base URL endpoint for the code generation LLM API.

    Environment variable: SimFramework_CODER_LLM_API_BASE
    ...
    CODER_LLM_MODEL: str = os.getenv("SimFramework_CODER_LLM_MODEL", "glm-4.7")
    """
    Model identifier for code generation and programming tasks.

    Environment variable: SimFramework_CODER_LLM_MODEL
    Default: "glm-4.7"
    ...
    NANO_LLM_API_KEY: Optional[str] = (
        os.getenv("SimFramework_NANO_LLM_API_KEY") or LLM_API_KEY
    )
    """
    API key for the nano LLM service used in high-frequency operations.

    Environment variable: SimFramework_NANO_LLM_API_KEY
    Default: Falls back to LLM_API_KEY if not set
    ...
    NANO_LLM_API_BASE: str = os.getenv("SimFramework_NANO_LLM_API_BASE", LLM_API_BASE)
    """
    Base URL endpoint for the nano LLM API.

    Environment variable: SimFramework_NANO_LLM_API_BASE
    Default: Falls back to LLM_API_BASE if not set
    ...
    NANO_LLM_MODEL: str = os.getenv(
        "SimFramework_NANO_LLM_MODEL", "qwen3-next-80b-a3b-instruct"
    )
    """
    Model identifier for high-frequency, low-latency operations.

    Environment variable: SimFramework_NANO_LLM_MODEL
    Default: "qwen3-next-80b-a3b-instruct"
    ...
    EMBEDDING_API_KEY: Optional[str] = (
        os.getenv("SimFramework_EMBEDDING_API_KEY") or LLM_API_KEY
    )
    """
    API key for the embedding model service.

    Environment variable: SimFramework_EMBEDDING_API_KEY
    Default: Falls back to LLM_API_KEY if not set
    ...
    EMBEDDING_API_BASE: str = os.getenv("SimFramework_EMBEDDING_API_BASE", LLM_API_BASE)
    """
    Base URL endpoint for the embedding API service.

    Environment variable: SimFramework_EMBEDDING_API_BASE
    Default: Falls back to LLM_API_BASE if not set
    ...
    EMBEDDING_MODEL: str = os.getenv("SimFramework_EMBEDDING_MODEL", "bge-m3")
    """
    Model identifier for text embedding generation.

    Environment variable: SimFramework_EMBEDDING_MODEL
    Default: "bge-m3"
    ...
    EMBEDDING_DIMS: int = int(os.getenv("SimFramework_EMBEDDING_DIMS", "1024"))
    """
    Dimensionality of the embedding vectors produced by the embedding model.

    Environment variable: SimFramework_EMBEDDING_DIMS
    Default: 1024
        ...
        if model_type == "coder":
            ...
            if not coder_api_key:
                raise ValueError(
                    ...
                    "Set SimFramework_CODER_LLM_API_KEY or SimFramework_LLM_API_KEY"
                )
            if not default_api_key:
                raise ValueError(
                    "API key not configured for default model (fallback). "
                    "Set SimFramework_LLM_API_KEY"
                )
            if not nano_api_key:
                raise ValueError(
                    "API key not configured for nano model (fallback). "
                    "Set SimFramework_NANO_LLM_API_KEY or SimFramework_LLM_API_KEY"
                )
            ...
            if not api_key:
                raise ValueError(
                    ...
                    f"Set SimFramework_{model_type.upper()}_LLM_API_KEY or SimFramework_LLM_API_KEY"
                )
