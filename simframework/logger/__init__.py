"""
Create logger named SimFramework as singleton for ray.
"""
...
def get_logger():
    logger = logging.getLogger("SimFramework")
...
def setup_logging(
    ...
    Returns:
        Configured SimFramework logger instance
    """
    # Default format
    ...
    # Configure SimFramework logger
    logger = get_logger()
    logger.setLevel(log_level)
    logger.handlers.clear()
    logger.propagate = False
    
    # Add handlers to SimFramework logger
    if log_file:
        file_handler = logging.FileHandler(log_file, mode="w", encoding="utf-8")
        file_handler.setLevel(log_level)
        file_handler.setFormatter(file_formatter)
