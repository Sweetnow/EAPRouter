...
from eaprouter.agent import AgentBase
from eaprouter.config import get_llm_router_and_model
from eaprouter.env import RouterBase
from eaprouter.logger import get_logger

__all__ = ["EAPRouterHelper"]
...
class EAPRouterHelper:
    ...
    async def _run(self, text: str, readonly: bool) -> str:
        ...
        print(f"\n{'='*60}")
        print(f"EAPRouterHelper Starting ({mode} Mode)")
        print(f"Task: {text}")
        print(f"{'='*60}\n")
    ...
    def _build_planning_prompt(self, task: str, readonly: bool) -> str:
        ...
        return f"""# Task Planning for EAPRouterHelper

## Objective
You are a planning assistant for EAPRouterHelper. Your goal is to create a step-by-step execution plan for {mode}.
...
Now, create a plan for the given task. Your JSON response:"""
    ...
    def _build_replanning_prompt(
        ...
        return f"""# Plan Adjustment for EAPRouterHelper
...
Create an updated plan. Your JSON response:"""
    ...
    def _build_summary_prompt(
        ...
        return f"""# Final Answer Generation for EAPRouterHelper
